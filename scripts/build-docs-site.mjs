...
function homeHero(page) {
  ...
  return `<header class="home-hero">
        ...
        <div class="home-cta">
          ...
          <div class="home-install" aria-label="Install from npm">
        ...
        <p class="muted"><a href="${installRel}">Install options &amp; bundlers &rarr;</a></p>
      </header>`;
}
    ...
    <aside class="sidebar">
      <div class="sidebar-head">
        <a class="brand" href="${hrefToOutRel("index.html", page.outRel)}" aria-label="${productName} docs home">
          ...
          <span><strong>${escapeHtml(productName)}</strong><small>Opus codec for JS</small></span>
        </a>
        ${themeToggleHtml()}
      </div>
      <label class="search"><span>Search</span><input id="doc-search" type="search" placeholder="encode, decode, fec"></label>
...
function highlightCode(code, lang) {
  ...
  if (language === "html" || language === "xml") return escapeHtml(code);
...
function restoreStashTokens(value, stash) {
  return value.replace(new RegExp("[" + String.fromCharCode(0xe000) + "-" + String.fromCharCode(0xf8ff) + "]", "g"), (token) => {
...
function highlightShellLine(line) {
  ...
  working = working.replace(/\b(npm|npx|pnpm|yarn|node|git|gh|emcc|emconfigure|emmake|emsdk|curl|wget|jq|cd|export|cat|ls|mv|cp|rm|mkdir|make|tar|sh|bash)\b/g, (m) => stashAdd(m, "hl-cmd"));
...
function highlightJs(code) {
  return withStash(code, [
    ...
    [/"(?:\\.|[^"\\])*"/g, "hl-s"],
    [/'(?:\\.|[^'\\])*'/g, "hl-s"],
    [/\b(const|let|var|function|return|if|else|for|while|switch|case|break|continue|class|extends|new|import|from|export|default|async|await|try|catch|finally|throw|typeof|instanceof|interface|type|enum|as|of|in|null|undefined|true|false|this|using)\b/g, "hl-k"],
    [/\b(\d+(?:\.\d+)?(?:_\d+)*)\b/g, "hl-n"],
  ]);
}
